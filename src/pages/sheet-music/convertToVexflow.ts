...
import type {Measure, Note, Beat, Duration, DrumNoteInstrument} from './drumTypes';
import {mapping, convertNoteToDrumInstrument} from './drumTypes';

// Re-export types so existing consumers continue to work
export type {Measure, Note, Beat, Duration, DrumNoteInstrument};
export {mapping} from './drumTypes';
          ...
          if (
            ...
            beat.notes.push({
              notes: noteGroups[noteGroupIndex].map(
                note => mapping[convertNoteToDrumInstrument(note)],
              ),
              isRest: false,
              dotted: false,
              isTriplet: false,
